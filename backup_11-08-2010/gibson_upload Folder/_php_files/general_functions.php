<?php
	...
	function get_unique_folder($row_rs_schema,$totalRows_rs_schema,$rs_schema,$main_db,$conn_main_db,$tablename,$keyfieldvalue)
		{
			mysql_select_db($main_db, $conn_main_db);
			for ($i = 0;$i<$totalRows_rs_schema;$i++)
				{
					if ($row_rs_schema['field_type'] == "picture")
						{
							...
							//l_name,f_name. 
							$dyn_fldr_nme = $row_rs_schema['dynamic_folder_name'];
							//Explode this data to see if there is more than one item in this field.
							$field_stuff = explode(",",$row_rs_schema['dynamic_folder_name']);
							$field_table = $row_rs_schema['dynamic_folder_table'];
						}
					$row_rs_schema = mysql_fetch_assoc($rs_schema);	
				}
							...
							if (!($field_table == "this")&&!($field_table == "This"))
								{
									...
									for ($i = 0;$i<$totalRows_rs_schema;$i++)
										{
											if ($row_rs_schema['xref_record_field'] == $dyn_fldr_nme)
												{
													//echo("dyn fldr nme = ".$dyn_fldr_nme."<BR>");
													if (isset($_POST[$row_rs_schema['field_name']]))
														{
															$id = $_POST[$row_rs_schema['field_name']];
														}
													else if (isset($_GET[$row_rs_schema['field_name']]))
														{
															$id = $_GET[$row_rs_schema['field_name']];
														}
													else
														{
															...
															$row_rs_temp = mysql_fetch_assoc($rs_temp);
															$id = $row_rs_temp[$row_rs_schema['field_name']];
														}
													$select_SQL = "SELECT * FROM ".$row_rs_schema['xref_table']." WHERE id_number = ".$id.";";
