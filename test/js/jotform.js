...
var JotForm = {
    ...
    */
    ...
    takeScreenShot: function(id){
        ...
        this.wishboxServer = '//screenshots.jotform.com/wishbox-server.php';
    ...
    checkScreenShot: function(){
        ...
        p.$jot.getJSON('//screenshots.jotform.com/queue/'+this.scuniq+'?callback=?',
            function(data) {
        ...
        p.$jot.getJSON(this.server+'?callback=?', {
            ...
            function(res) {
                ...
                if(ie !== undefined && ie < 9){
                    ...
                    editorFrame.attr('src', '//screenshots.jotform.com/opt/templates/screen_editor.html?shot='+url+'&uniq='+$this.scuniq);
                    ...
                    b.click(function(){
                        ...
                        p.$jot.getJSON('//screenshots.jotform.com/wishbox-server.php?callback=?', {
                            action: 'getImage',
                            uniqID: $this.scuniq
                        },function(res){
                            if(!res.success){
                ...
                try{
                    ...
                    switch (question.type) {
                        ...
                        case "control_matrix":
                            ...
                            //Emre session does not work on "matrix" (60981)
                            if(question.name == undefined) {
                            	var firstElementInMatrix = $$("#id_" + qid + ' input')[0] || $$("#id_" + qid + ' select')[0];
                            	var questionTmpName = firstElementInMatrix.name;
                            	
                            	if(firstElementInMatrix.type == "radio"){
                            		question.name = questionTmpName.substr(3,questionTmpName.length-6);                            	
                            	}else{
                            		question.name = questionTmpName.substr(3,questionTmpName.length-8);
                            	}
                            }
                    ...
                    if (vals.include("Email_Confirm")) {
                        ...
                        if (input.tagName == 'INPUT' && input.readAttribute('type') == "file") { // Upload
                            ...
                            try{
                                ...
                                //Emre: "birthDay picker" element required does not work because only "input" selected (68106)
                                var typeOfElement = input.name.include('birthDate') ? "select" : "input";
                                var checkValues = cont.select(typeOfElement + '[name*=' + input.name.replace(/\[.*$/, '') + ']').map(function(e){
